...
class DatabaseInterface(Protocol):
    ...
    внутри (таблицы, строки, SQL) — деталь реализации. Жизненный цикл соединения
    (открытие и закрытие) — тоже деталь адаптера, в порт не входит: соединением
    управляет ``async with`` вокруг адаптера. Схема БД (таблицы и миграции) —
    забота реализации: её накатывает Alembic, порт об этом не знает.
    """
