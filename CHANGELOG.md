...
- Схема БД больше не создаётся приложением: из `SqliteAdapter` убраны константа `SCHEMA` и метод `_migrate()`; при старте адаптер только проверяет, что миграции накатаны (таблица `alembic_version`), и падает с понятным сообщением `выполните alembic upgrade head`, если это не так.
...
- Публичные `init()` и `shutdown()` у `SqliteAdapter` — соединение с БД открывается и закрывается контекстным менеджером (`async with container.sqlite_adapter():` в `main.py`), снаружи дёргать жизненный цикл больше не нужно.
